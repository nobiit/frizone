	...
	methods: {
			...
			$(_self.target).ajaxSubmit({
				...
				success: function(data) {
					if (data.status == 200) {
						...
						else if(FriZone.curr_pn == "thread" && _app_.thread_id) {
							_app_.thread_id     = 0;
		...
		record_audio_stop: function() {
			...
			_app_.audio_rec.recorder.exportWAV(function(blob) {
			    ...
			    if (FriZone.max_upload(file_data.size)) {
		...
		upload_images: function(event = null) {
			...
			if (cl_empty(_app_.active_media) || _app_.active_media == 'image') {
				...
				if (FriZone.curr_pn == 'thread') {
	        		$('div[data-app="modal-pubbox"]').addClass('vis-hidden');
	        	}

				FriZone.progress_bar("show");

				if (images.length) {
					for (var i = 0; i < images.length; i++) {
						if (FriZone.max_upload(images[i].size)) {
		...
		upload_video: function(event = null) {
			...
			if (cl_empty(_app_.active_media)) {
				...
				if (video && FriZone.max_upload(video.size)) {
					...
					$.ajax({
						...
						success: function(data) {
							if (data.status == 200) {
								...
								if (data.error) {
									FriZone.cl_bs_notify(data.error, "danger");
								}
								else{
									FriZone.errorMSG();
								}
